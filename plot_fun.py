...
def plot_aerofoil(jou, vortex_cor_x, vortex_cor_y,i):
    heading = str(i)
    jou_x = jou.real
    jou_y = jou.imag
    ...
    plt.plot(jou_x, jou_y, color = 'b')
    plt.gca().set_aspect('equal', adjustable='box')
    plt.scatter(vortex_cor_x, vortex_cor_y, s=10, color = 'g')
    plt.savefig(heading)
    plt.close()
...
def transpose_array(x_array, y_array, aoa):
    ...
    for i in range(len(x_array)):
        com = complex(x_array[i], y_array[i])
        new = np.exp(- 1j * aoa) * com
        
        x_array_new.append(new.real)
        y_array_new.append(new.imag)
